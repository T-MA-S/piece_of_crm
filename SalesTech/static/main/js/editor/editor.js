...
(function( $ ){
	...
	var methods = {
		...
		init : function( options )
		{
			...
			var customvariables = JSON.parse(this[0].dataset.variables);
						...
						'insert_table'	: { "modal": true,
						   ...
						   'outdent'	: { "text": "Outdent", 
							...
							'custom_variables': { "text": "{ }",
											"tooltip": "Insert Custom Variable",
											"commandname":null,
										"custom": function(button){
													methods.restoreIESelection.apply(this);
													var flag =0;
													var splCharDiv = $('<div/>',{id:"customvariable", class:"customVariablesCntr", css :{"display":"none"}}).click(function(event) { event.stopPropagation();});
													var splCharUi  = $('<ul />',{id:"special_ui"});
													var editor_Content = this;
													if($(this).data("editor").data("cstmVrblsBtn")){
														flag = 1;
														$(this).data("editor").data("cstmVrblsBtn", null);
													}
													else
														$(this).data("editor").data("cstmVrblsBtn", 1);

													if(flag==0){
														for (var i = 0; i < customvariables.length; i++){
															splCharUi.append($('<li />').html(customvariables[i].name).attr('title',customvariables[i].name).attr('data-value', `{{${customvariables[i].variable}}}`).mousedown(function(event){ event.preventDefault();}).click(function(event){
																if(navigator.userAgent.match(/MSIE/i) || navigator.userAgent.match(/Windows NT.*Trident\//)){
																	var specCharHtml = $(this).attr( "data-value" );
																	methods.insertTextAtSelection.apply(this,[specCharHtml,'html']);
																}
																else{
																	document.execCommand('insertHTML',false,$(this).attr( "data-value" ));
																}
																$('#specialchar').remove();
																$(editor_Content).data("editor").data("cstmVrblsBtn", null);
															}));
														}
														splCharUi.prependTo(splCharDiv);
														splCharDiv.insertAfter(button)
														$('#customvariable').slideDown('slow');
													}
													else
			...
			var menuGroups = {'texteffects' : ['bold', 'italics', 'underline', 'color'],
							  ...
							  'extraeffects' : ['strikeout', 'hr_line', 'splchars', 'custom_variables'],
			...
			var statusBar = $("<div/>", {	id : "statusbar_" + $(this).attr("id"),
	       	...
	       	if(settings['status_bar']){
				editor.keyup(function(event){
					...
					$(editor_Content).data("statusBar").html('<div class="label">'+'Characters : '+charCount+'</div>');
            	});
	        }	        
		    ...
		    $(document.body).mousedown(function(event) {
				...
				if (!target.parents().addBack().is('#customvariable') && (target.closest('a').html()!='<i>{ }</i>')) { //Clicked outside
		        	if($("#customvariable").is(':visible'))
		            {
						$(editor_Content).data("editor").data("cstmVrblsBtn", null);
						$('#customvariable').remove();
		           	}
